...
class metadata_xml_T : public xml::Document<MetadataXMLHandler_T>
{
    public:
        ...
        metadata_xml_T(const string_type &s)
            : xml::Document<MetadataXMLHandler_T>(s) { }
        virtual ~metadata_xml_T() { }

        /* handler_type access functions */
        void display(const string_type &p) const
        { this->data(p).display(); }
        herds_type  &herds()    const { return this->handler()->data.herds; }
        herd_type   &devs()     const { return this->handler()->data.devs;  }
        string_type &longdesc() const { return this->handler()->data.longdesc; }
        metadata_T  &data(const string_type &portdir) const
        {
            this->handler()->data.path = this->path();
            this->handler()->data.portdir = portdir;
            this->handler()->data.get_pkg_from_path();
            return this->handler()->data;
        }

        bool is_category() const { return this->handler()->data.is_category; }
        bool dev_exists(const herd_type::key_type &dev) const
        { return this->handler()->data.dev_exists(dev); }
        bool dev_exists(const util::regex_T &re) const
        { return this->handler()->data.dev_exists(re); }
        bool herd_exists(const herds_type::value_type &herd) const
        { return this->handler()->data.herd_exists(herd); }
        bool herd_exists(const util::regex_T &re) const
        { return this->handler()->data.herd_exists(re); }
};

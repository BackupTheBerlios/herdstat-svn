...
{
    std::memset(_handler.get(), 0, sizeof(struct xmlSAXHandler));
...
SAXParser::parse()
{
    ...
    if (not _context->wellFormed)
        throw ParseError("not well formed");
...
{
    if (_context)
        throw ParseError("Attempting to instantiate a second parser context");

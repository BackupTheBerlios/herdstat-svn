/*
 * herdstat -- src/action_keywords_handler.cc
 * $Id: action_versions_handler.cc 664 2005-10-05 11:27:12Z ka0ttic $
    ...
    if (opts.empty())
    {
        ...
        if (not portage::in_pkg_dir())
        {
            std::cerr << "You must be in a package directory if you want to run" << std::endl
                << PACKAGE << " --keywords with no non-option arguments." << std::endl;
            return EXIT_FAILURE;
        }

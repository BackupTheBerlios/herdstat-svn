...
stat_T::operator() ()
{
    if (this->_opened)
    {
        /* TODO: find out how to get the file descriptor
         * from the stream for fstat. */
    }
    else
    {
...
template <class C>
bool
util::base_file_T<C>::operator== (const base_file_T<C> &that)
{
    if (this->size() != that.size())
        return false;

    /* is every single element equal? */
    std::pair<iterator, const_iterator> p;
...
template <class C>
void
util::base_file_T<C>::dump(std::ostream &os) const
{

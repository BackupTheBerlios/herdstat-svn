                      ...
                      bool overlays, util::timer_T *timer)
{
    ...
    if (p.first != portdir)
    {
        ...
        try
        {
            e1 = ebuild_which(portdir, p.second, timer);
            e2 = ebuild_which(p.first, p.second, timer);
        }
        catch (const nonexistent_pkg_E &e)
        {   e2 = ebuild_which(p.first, p.second, timer); }

        if (e1.empty()) return e2;
        else
        {
            version_string_T v1(e1), v2(e2);
            return ((v1>v2)? e1:e2);
        }
    }
    else
